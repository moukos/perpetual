...
static void check_globals(ctx_t *_a) {
  ...
  int *x = _a->x;
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("sb-iwp2.3.a-amd4, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("sb-iwp2.3.a-amd4, check_globals failed");
  }
...
static void *P0(void *_vb) {
  struct timespec start, barr, end;
  double time_sync = 0.0, time_test = 0.0;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test sb-iwp2.3.a-amd4 Allowed\n");
  ...
  fprintf(out,"Hash=edd4722437d708675ed921e7607e77f0\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation sb-iwp2.3.a-amd4 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  if (p_true > 0) {
  }
  fprintf(out,"Time sb-iwp2.3.a-amd4 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "sb-iwp2.3.a-amd4: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
    log_error("\n");
  }
  hist_t *hist = NULL;
  int n_th = n_exe-1;
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("sb-iwp2.3.a-amd4, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }

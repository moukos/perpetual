...
int main(int argc,char *argv[]) {
    ...
    time_h = (double) end.tv_sec + (double) end.tv_nsec / 1000000000.0 - (double)start.tv_sec - (double) start.tv_nsec / 1000000000.0;
   
    // Prepare synchronized version
    arg_t0.num_iterations = arg_t1.num_iterations = arg_t2.num_iterations = arg_t3.num_iterations = 1;
    ...
    for(loop = 0; loop < n; loop++){
 	clock_gettime(CLOCK_MONOTONIC_RAW, &start);
    	...
    	clock_gettime(CLOCK_MONOTONIC_RAW, &end);
    	time_sync = (double) end.tv_sec + (double) end.tv_nsec / 1000000000.0 - (double)start.tv_sec - (double) start.tv_nsec / 1000000000.0;
	overhead += time_sync;
    }

...
int main(int argc,char *argv[]) {
    ...
    long oldCnt = 0;
    long adjustedInterleavingsCnt = 0; 
    double time_n = 0;
    // Checker - Base
    if( n <= 10000 ){
    	clock_gettime(CLOCK_MONOTONIC_RAW, &start);
    	interleavingsCnt = condition(arg_t0.buf,arg_t1.buf,arg_t2.buf,arg_t3.buf,n,&adjustedInterleavingsCnt);
    	clock_gettime(CLOCK_MONOTONIC_RAW, &end);
    ...
    printf("%d %d %d %ld %.9f %.9f %.9f %.9f\n", n,  interleavingsCnt, adjustedInterleavingsCnt, interleavingsCnt2, (double) interleavingsCnt2/adjustedInterleavingsCnt,time_h, time_n, time_n2);
    fprintf(stderr, "%d %d %d %ld %.9f %.9f %.9f %.9f\n", n,  interleavingsCnt, adjustedInterleavingsCnt, interleavingsCnt2, (double) interleavingsCnt2/adjustedInterleavingsCnt, time_h, time_n, time_n2);

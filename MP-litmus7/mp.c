...
static void check_globals(ctx_t *_a) {
  ...
  int *x = _a->x;
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("mp-iwp2.1-amd1, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("mp-iwp2.1-amd1, check_globals failed");
  }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test mp-iwp2.1-amd1 Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "mp-iwp2.1-amd1: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
    log_error("\n");
  }
  hist_t *hist = NULL;
  int n_th = n_exe-1;

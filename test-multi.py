...
testname_strip = testname.split("_")
litmusname = testname_strip[0]
...
perpleFile = "PerpLETestSuitesChecked/x86tso-multi-unfenced/" + testname + "/num_reads.perple"
checkerFile = "PerpLETestSuitesChecked/x86tso-multi-unfenced/" + testname + "/checker.c"
checkerHeuristicFile = "PerpLETestSuitesChecked/x86tso-multi-unfenced/" + testname + "/checker-heuristic.c"
...
threadfilenames = "./PerpLETestSuitesChecked/x86tso-multi-unfenced/" + testname + "/" + litmusname + "_thread_*"
...
# create folder
pathname = "./PerpLEResults/x86tso-multi-unfenced/" + testname
print(pathname)
try:
...
# Open summary file
summfilename = "./PerpLEResults/x86tso-multi-unfenced/" + testname + "/" + "summary_" + testname + "_" + now.strftime("%Y-%m-%d_%H:%M:%S")
...
n = start
for j in range(steps):
    subprocess.call(["litmus", "./LitmusTestSuites/x86tso/" + litmusname + ".litmus", "-r", str(tries), "-s", str(n)], stdout=f)
    n = multi * n
f.close()
